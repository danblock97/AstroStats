...
async def pet_battle(interaction: discord.Interaction, opponent: discord.Member):
    ...
    if (opponent_pet['level'] > user_pet['level'] + 1
        ...
        embed = discord.Embed(
            title="Battle Error",
            description=(
                "You can only battle someone with a pet that is higher level, "
                "the same level, or one level below yours."
            ),
            color=discord.Color.red()
        )
        await interaction.response.send_message(embed=embed)

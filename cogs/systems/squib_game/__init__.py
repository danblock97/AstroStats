...
async def get_guild_avatar_url(guild: discord.Guild, user_id: int) -> Optional[str]:
    """Get a user's guild avatar URL or their default avatar URL."""
    try:
...
def play_minigame_logic(round_number: int, participants: list) -> tuple[list, dict]:
    """Execute the logic for a minigame round and return updated participants."""
...
def generate_flavor_text(minigame_desc: str, eliminated_this_round: list, alive_players: list) -> str:
    """Generate descriptive text for the round's results."""
    flavor_sentences = []
    max_display = 10
...
def create_new_session(guild_id: str, user_id: str, display_name: str) -> Tuple[str, dict]:
    ...
    session_id = f"{guild_id}_{user_id}_{int(datetime.datetime.now().timestamp())}"
    new_session_doc = {
        "guild_id": guild_id,
...
async def conclude_game_auto(bot, interaction, game_doc: dict, guild_id: str, final_round: int, winner=None) -> list:
    """Conclude a game and determine the winner."""
    ...
    # Update winner stats
    user_stats = squib_game_stats.find_one({"user_id": winner["user_id"], "guild_id": guild_id})
    if not user_stats:
        new_stats = {
        ...
        new_wins = user_stats.get("wins", 0) + 1
...
async def run_game_loop(bot, interaction, game_id: str, guild_id: str):
    """Run the main game loop for a Squib Game."""
    while True:
...
class SquibGames(commands.GroupCog, name="squibgames"):
    ...
    async def run(self, interaction: discord.Interaction):
        ...
        if game["current_game_state"] == "waiting_for_players":
            ...
            run_initiator = interaction.user.mention
            await interaction.response.send_message(f"**{run_initiator}** has initiated the automatic rounds...",
                                                    ephemeral=True)
    ...
    async def status(self, interaction: discord.Interaction):
        ...
        embed.add_field(name="💀 Eliminated Players", value=summarize_list(eliminated_list, "Eliminated Players"),
                        inline=False)

...
from .petbattle import calculate_damage
...
class PetBattles(commands.GroupCog, name="petbattles"):
    ...
    async def check_user_vote(self, user_id):
        """
        Safe wrapper around the topgg client's get_user_vote method.
        ...
        try:
            ...
            has_voted_raw = await self.topgg_client.get_user_vote(user_id)
            logger.debug(f"Vote response: {has_voted_raw}, type: {type(has_voted_raw)}")

            # If it's a boolean already, return it directly
            if isinstance(has_voted_raw, bool):
                return has_voted_raw

            # If it's an integer, convert to boolean
            if isinstance(has_voted_raw, int):
                return bool(has_voted_raw)

            # If it's a string, try parsing it
            if isinstance(has_voted_raw, str):
                # If it's just "1" or "0", convert directly
                if has_voted_raw.strip() in ["0", "1"]:
                    return has_voted_raw.strip() == "1"

                # Try to parse as JSON
                try:
                    parsed = json.loads(has_voted_raw)
                    if isinstance(parsed, dict):
                        return bool(parsed.get("voted", 0))
                    else:
                        return bool(parsed)
                except json.JSONDecodeError:
                    # Not JSON, just check if the string is truthy
                    return bool(has_voted_raw)

            # If it's a dict, look for the voted key
            if isinstance(has_voted_raw, dict):
                return bool(has_voted_raw.get("voted", 0))

            # Default: convert to boolean
            return bool(has_voted_raw)

        except TypeError as e:
            # If we get TypeError (string indices), try to handle it manually
            logger.warning(f"Error in topgg client: {e}")

            # Import aiohttp here to ensure it's available
            import aiohttp

            # Direct API call using a new aiohttp session
            try:
                # Get bot ID from the bot object
                bot_id = str(self.bot.user.id)
                base_url = "https://top.gg/api"

                # Use direct TOPGG_TOKEN from environment/settings
                headers = {"Authorization": TOPGG_TOKEN}

                # Create a temporary session
                async with aiohttp.ClientSession() as session:
                    async with session.get(
                            f"{base_url}/bots/check?userId={user_id}",
                            headers=headers
                    ) as resp:
                        if resp.status != 200:
                            logger.error(f"TopGG API error: {resp.status}")
                            return False

                        data = await resp.json()
                        return bool(data.get("voted", 0))
            except Exception as e2:
                logger.error(f"Failed to check vote status manually: {e2}", exc_info=True)
                return False
        ...
        try:
            ...
            if not opponent_pet:
                embed = discord.Embed(
                    ...
                    description=(
                        f"{opponent.mention} doesn't have a pet in this server. They need to summon one first."),
                    color=discord.Color.red()
                )
                await interaction.response.send_message(embed=embed)
                return
            ...
            completed_quests_user, completed_achievements_user = update_quests_and_achievements(user_pet,
                                                                                                user_battle_stats)
            completed_quests_opponent, completed_achievements_opponent = update_quests_and_achievements(opponent_pet,
                                                                                                        opponent_battle_stats)
            ...
            if completed_quests_user or completed_achievements_user:
                ...
                for quest in completed_quests_user:
                    completion_message += (
                        f"📝 **Quest Completed**: {quest['description']} (+{quest['xp_reward']} XP)\n")
                for achievement in completed_achievements_user:
                    completion_message += (
                        f"🏆 **Achievement Unlocked**: {achievement['description']} (+{achievement['xp_reward']} XP)\n")
            ...
            if user_leveled_up or opponent_leveled_up:
                ...
                if user_leveled_up:
                    level_up_message += (
                        f"{interaction.user.display_name}'s pet leveled up to **Level {user_pet['level']}**!\n")
                if opponent_leveled_up:
                    level_up_message += (
                        f"{opponent.display_name}'s pet leveled up to **Level {opponent_pet['level']}**!\n")
                battle_embed.description += f"\n{level_up_message}"
    ...
    async def quests(self, interaction: discord.Interaction):
        ...
        try:
            ...
            if not incomplete_quests:
                now = datetime.now(timezone.utc)
                next_reset = datetime.combine(now.date(), dtime(hour=0, minute=0, tzinfo=timezone.utc)) + timedelta(
                    days=1)
    ...
    async def achievements(self, interaction: discord.Interaction):
        ...
        try:
            ...
            for achievement in achievements_list:
                status = "✅ Completed" if achievement[
                    'completed'] else f"{achievement['progress']} / {achievement['progress_required']}"
    ...
    async def vote(self, interaction: discord.Interaction):
        ...
        try:
            ...
            logger.error(f"Error in vote command: {e}", exc_info=True)
            embed = discord.Embed(
                title="Error",
